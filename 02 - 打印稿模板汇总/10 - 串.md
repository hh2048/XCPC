...
给定一个字符串 $S$，求解其某个子串 $S[l \dots r]$ 独立/非独立出现的次数。例如，对于原串 $S={\tt"aaaaa"}$，询问 $S[1\dots 2]$ 独立出现的次数答案为 $2$，非独立答案为 $4$，需要注意原串需要添加前缀空白字符，下标从 $1$ 开始，其本质是模板的带未知变量的扩展版本。

```c++
auto KMP = [&](int l, int r) -> int {
    int ans = 0;
    vector<int> nxt(r + 1);
    nxt[l] = l - 1;
    for (int i = l + 1, j = l - 1; i <= r; i++) {
        while (j != l - 1 && s[i] != s[j + 1]) {
            j = nxt[j];
        }
        if (s[i] == s[j + 1]) j++;
        nxt[i] = j;
